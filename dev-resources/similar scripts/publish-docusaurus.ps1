...
$docusaurusConfigPath = Join-Path $docsSitePath 'docusaurus.config.ts'
...
$expectedBaseUrl = "baseUrl: '/$SitePath/'"
$expectedUrl = "url: '$PublicBaseUrl'"
...
if (-not (Test-Path $docusaurusConfigPath))
{
    throw "Docusaurus config not found: $docusaurusConfigPath"
}

$docusaurusConfig = Get-Content -LiteralPath $docusaurusConfigPath -Raw
if ($docusaurusConfig -notmatch [regex]::Escape($expectedUrl))
{
    throw "docs-site/docusaurus.config.ts must contain $expectedUrl before publishing."
}

if ($docusaurusConfig -notmatch [regex]::Escape($expectedBaseUrl))
{
    throw "docs-site/docusaurus.config.ts must contain $expectedBaseUrl before publishing."
}
